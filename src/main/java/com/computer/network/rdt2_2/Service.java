			...
			while (buffer.hasRemaining()) {
				...
				if (c == '\n') {
					...
					try {
						...
						Packet sndpkt = null;
						if (!receiver.rdt_rcv(rcvpkt) && receiver.notcorrupt(rcvpkt) && receiver.has_seq(rcvpkt, seq)) {
							int checksum = sender.check_sum("ACK");
							sndpkt = sender.make_pkt(seq, "ACK", checksum);
							seq = seq ^ 1;
						} else if (receiver.rdt_rcv(rcvpkt) && receiver.corrupt(rcvpkt)
								&& receiver.has_seq(rcvpkt, seq)) {
							int checksum = sender.check_sum("ACK");
							sndpkt = sender.make_pkt(seq ^ 1, "ACK", checksum);
						}
