...
public class RaftParameters {
	...
	/**
	 * For new member that just joined the cluster, we will use log sync to ask
	 * it to catch up, and this parameter is to tell when to stop using log sync
	 * but appendEntries for the new server when leaderCommitIndex -
	 * indexCaughtUp < logSyncStopGap, then appendEntries will be used
	 * 
	...
	/**
	 * For new member that just joined the cluster, we will use log sync to ask
	 * it to catch up, and this parameter is to specify how many log entries to
	 * pack for each sync request
	 * 
	...
	/**
	 * The maximum heartbeat interval, any value beyond this may lead to
	 * election timeout for a peer before receiving a heartbeat
	 * 
	 * @return
	 */
	...
	/**
	 * the max gap allowed for log sync, if the gap between the client and
	 * leader is less than this value, the ReplicateLogRequest will be stopped
	 * 
	 * @return
	 */
